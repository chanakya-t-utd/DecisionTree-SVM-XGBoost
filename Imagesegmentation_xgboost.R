...
fact<-as.factor(c("GRASS","PATH","WINDOW","CEMENT","FOLIAGE","SKY","BRICKFACE"))
...
df$CLASS<-vlookup(fact,vals,df$CLASS)
...
y<-trainMatrix[,1] #is the target variable, extracted out as it needs to be passed as parameter into the xgboost function.
...
pred.cv = matrix(bst.cv$pred, nrow=length(bst.cv$pred)/7, ncol=7) #make a 7*7 matrix of probabilities for each  cluster.
